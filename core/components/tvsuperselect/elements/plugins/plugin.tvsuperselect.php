...
if(!($tvss instanceof tvSuperSelect)) return '';
...
switch ($modx->event->name)
{
	...
	case 'OnDocFormSave':
		if (is_object($resource) && is_array($resource->_fields))
		{
			...
			if(!empty($flds))
			{
				// $modx->log(modX::LOG_LEVEL_ERROR, print_r($flds, 1));
				...
				foreach($flds as $fld)
				{
					...
					foreach ($fld['data'] as $value)
					{
						if (!empty($value))
						{
							$values[] = '('. $fld['resource_id'] .',"'. $fld['tv_id'] .'","'.$value.'")';
						}
					}
			...
			if(!empty($tv_values))
			{
				// $modx->log(modX::LOG_LEVEL_ERROR, print_r($tv_values, 1));
				...
				foreach ($tv_values as $tv_id => $values)
				{
					...
					$tv_obj->save();
					// $modx->log(modX::LOG_LEVEL_ERROR, print_r($tv_obj->toArray(), 1));
